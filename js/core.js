...
(function(global) {
  ...
  W.css = (function() {
    ...
    return function(cssPath) {
      ...
      var config;
      if (typeof(config = args.pop()) == 'object') {
        for (var i in cssDefaultConfig) {
          config[i] = config[i] || cssDefaultConfig[i];
        }
      } else {
        args.push(config);
        config = cssDefaultConfig;
      }
  ...
  document.writeln('<script src="'+_resolve('./version.js',coreJsPath)+'?'+Math.random()+'"></script>');//得到版本号并回调处理
  ...
  global.__coreCallback = function(frontVersion){
    ...
    _seajs.config({
      ...
      alias: {
        'jquery': _resolve('./jquery-1.8.2.js',coreJsPath)
      },
      charset: 'utf-8'
    });
    //加载核心jquery
    W.use('jquery', function() {
      var newW = function(fn) {
        $(fn);
      }
      for (var i in W) {
        newW[i] = W[i];
