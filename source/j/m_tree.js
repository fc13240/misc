(function(){
	//from http://dftree.sourceforge.net/dftree/example-icon.html
	/** 
	 * @fileoverview
	 * Dynamic Folder Tree
	...
	var dFtreeProp = dFTreeDefaultLayout.prototype;
	/** 
	 * Insert in the document the HTML code for the root of the tree
	 * @param {dFTree} tree The reference tree
	 * @private
	 */
	dFtreeProp._draw_tree = function(tree,witeFn) {
		if (!getObjectById("dftree_"+tree.name))
		{
			(witeFn || document.write)('<table class="root" id="dftree_'+tree.name+'" cellspacing="0" cellpadding="0"></table>');
		}
	}
	 ...
	 * @private
	 */
	dFtreeProp._get_children_interval = function(node) {
		var startIndex = 0;
		var endIndex = 0;
	 ...
	 * @private
	 */
	dFtreeProp._draw_node = function(node)
	{
		var str;
		var div;
		...
		//zk add
		var _treeName = node._myTree.name;
		var _tree = dFTreeCache[_treeName];
		var _nodeId = node.id;
		var _node = _tree.getNodeById(_nodeId);
		...
		var table = getObjectById("dftree_"+_treeName);
	 ...
	 * @private
	 */
	dFtreeProp._update_on_context_menu= function(node, objL) {
	 ...
	 * @private
	 */
	dFtreeProp._refresh= function(node) {
		var _nodeId = node.id;
		var objN  = getObjectById("n"+_nodeId);
		var objP  = getObjectById("p"+_nodeId);
		var objL  = getObjectById("l"+_nodeId);
		var objI  = getObjectById("i"+_nodeId);
	 ...
	 * @private
	 */
	dFtreeProp._refresh_parent_vertical_lines = function(node, level, row) {
	 ...
	 * @private
	 */
	dFtreeProp._update_vertical_lines = function(node, objV)
	{
	 ...
	 * @private
	 */
	dFtreeProp._select_node = function(node)
	{
		var objL;
	 ...
	 * @private
	 */
	dFtreeProp._unselect_node = function(node)
	{
	 ...
	 * @private
	 */
	dFtreeProp._erase_node = function(node)
	{
	 ...
	 * @private
	 */
	dFtreeProp._clean_table = function(table, tree) {
	...
	var dNodeProp = dNode.prototype;
	/**
	 * Changes node state from open to closed, and vice-versa
	 */
	dNodeProp.changeState = function()
	{
		if (this._io)
		{
			this.close();
	...
	dNodeProp.open = function () {
		if (!this._io)
		{
			...
			if (!this._opened) 
			{
				var onFirstOpen = this.onFirstOpen;
				if (this.runJS && onFirstOpen != null)
				{
					onFirstOpen();
	 ...
	 */
	dNodeProp.close = function() {
		if (this._io)
		{
			this._myTree._closeBranch(this._children);
			var onClose = this.onClose;
			if (this.runJS && onClose != null)
			{
				onClose();
				// eval(this.onClose);
			}
	 ...
	 */
	dNodeProp.alter = function(arrayProps)
	{
		for (i in arrayProps)
		{
	 ...
	 * @private
	 */
	dNodeProp._properPlus = function()
	{
	 ...
	 * @private
	 */
	dNodeProp._properIcon = function()
	{
		if (!this._myTree.useIcons)
		{
			return "";
	...
	function dFTree(arrayProps) {
		...
		var i = this.name = dFTreeCache.i++;
		dFTreeCache[i] = this;
	}
	var dFTreeProp = dFTree.prototype;
	/**
	 * Insert in the HTML document in the current position the root HTML elements
	 */
	dFTreeProp.draw = function(writeFn) {
		if (this._drawn) return;
		this._drawn = true;
		var html = this.layout._draw_tree(this,writeFn);
	 ...
	 * @private
	 */
	dFTreeProp._drawBranch = function(childrenArray) {
		var a=0;
	 ...
	 * @private
	 */
	dFTreeProp._closeBranch = function(childrenArray) {
	 ...
	 * Close all nodes
	 */
	dFTreeProp.collapseAll = function() {
	 ...
	 */
	dFTreeProp.add = function(node,pid) {
		var auxPos;
		var addNode = false;
	 ...
	 */
	dFTreeProp.remove = function(node) {
		var childPos;
	 ...
	 */
	dFTreeProp.alter = function(arrayProps) {
		if (arrayProps['id'])
		{
	 ...
	 */
	dFTreeProp.getNodeById = function(nodeid) {
	 ...
	 * @private
	 */
	dFTreeProp._searchNode = function(id) {
	 ...
	 * @private
	 */
	dFTreeProp._searchNodeIn = function(id, list) {
		var a=0;
	 ...
	 * @private
	 */
	dFTreeProp._setCookie = function() {
	 ...
	 * @private
	 */
	dFTreeProp._getCookie = function() {
		if (this.followCookies)
		{
